...
\citep{Jarosik_2011}
$$H_0=70.50\pm2.37\, \mbox{(stat.)}\pm3.38\, \mbox{(sys.)} \,\mbox{km s}^{-1}\, \mbox{Mpc}^{-1}$$
\citet{hubble0}
\citep{scolnic2018}
...
Un resumen de tales soluciones se brinda en la Tabla \ref{tab:eqf}. \\

\begin{table}[!h]
\caption{Soluciones particulares a las ecuaciones de Friedmann.}
\[
\begin{array}{cccc}
\hline
\textbf{Componente dominante}&\omega&\rho&a(t)\\
\toprule
\mbox{Materia} & 0 & a^{-3}& (\frac{2}{3}H_0t)^{2/3}\\
\mbox{Radiaci\'on} & \frac{1}{3} & a^{-4}& (2H_0t)^{1/2}\\
\Lambda & -1 &\frac{\Lambda}{8 \pi G}& \mbox{exp}\left(\sqrt{\frac{\Lambda c^2}{3}}t\right)\\
\hline
\end{array}
\]
\label{tab:eqf}
